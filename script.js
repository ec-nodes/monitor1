...
async function fetchTransactions(node) {
    try {
        ...
        while (retryCount < MAX_RETRIES && !response) {
            try {
                const fetchResponse = await fetch(`https://bergsexplorer.premiumnodes.com/api?module=account&action=txlist&address=${node.nodeAddress}`);
...
newRow.innerHTML = `<td class="node-name">${nodeName}</td><td class="node-address"><a href="https://bergsexplorer.premiumnodes.com/address/${nodeAddress}" target="_blank">${newNodeAddressText}</a></td><td>${transactionTimeText}</td><td><img src="https://i.ibb.co/xHbVTPk/delete-3.webp" alt="Delete" class="delete-logo"></td>`;
...
function isValidCryptoAddress(address) {
    return /^[a-fA-F0-9]{40}$/.test(address);
        ...
        nodeAddressCell.innerHTML = `<a href="https://bergsexplorer.premiumnodes.com/address/${newAddress}">${newNodeAddressText}</a>`;
...
async function loadNodesData() {
    ...
    await Promise.all(storedNodes.map(async ({ nodeName, nodeAddress }) => {
        try {
            ...
            if (response) {
                const newNodeAddressText = generateNewNodeAddressText(nodeAddress);
                const row = table.querySelector(`tr td:nth-child(2) a[href="https://bergsexplorer.premiumnodes.com/address/${nodeAddress}"]`).parentNode.parentNode;
